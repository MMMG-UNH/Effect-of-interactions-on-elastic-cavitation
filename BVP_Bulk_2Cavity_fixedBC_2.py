...
This script solves the cavity-interface interaction problem: a single spherical cavity
near a rigid planar interface (modeled as a fixed bottom boundary). The bottom boundary
has zero normal displacement (u2=0) AND zero tangential displacement (u1=0), simulating
a perfectly bonded rigid substrate — in contrast to the symmetric BC version which only
enforces u2=0 at the bottom.

How to run:
    python3 BVP_Bulk_2Cavity_fixedBC_2.py A B C D E F G
    ...
    F: y_c             — dimensionless cavity center position d/R
    G: mesh_quality    — Coarse / Medium / Fine / Finest

Example:
    python3 BVP_Bulk_2Cavity_fixedBC_2.py 1 3374 15000 1 0 1.10 Finest
...
# y_c: dimensionless cavity center position = d/R (distance from bottom boundary / cavity radius)
...
YZero_boundary_surface = 13  # bottom edge — rigid interface (u2=0 AND u1=0 for fixed BC)
...
# u2=0 at bottom: zero vertical displacement (rigid interface — no normal penetration)
...
# u1=0 at bottom: FIXED BC — tangential slip is prevented at the rigid interface
# (this is the key difference from symmBC: at bottom, both u1 and u2 are fixed)
bcs_7 = DirichletBC(BVP.ME.sub(0).sub(0), 0, BVP.facets, YZero_boundary_surface)

# assemble the active set of boundary conditions based on loading mode
if (remote_displacement == 1 and remote_disp_uxfix == 0):
    # remote loading with free horizontal motion at top: fix axis, bottom (u1+u2), right
    bcs = [bcs_1, bcs_2, bcs_3, bcs_5, bcs_6, bcs_7]
else:
...
while (t < T_f):
    ...
    if (ii - 1) % 10 < 0.1:
        ...
        if (remote_displacement == 1):
            print(
                "Step: Remote displacement |   Simulation Time: {}    |     Wallclock Time: {}".format(t, current_time))
        else:
            print("Step: Inner displacement   |   Simulation Time: {}    |     Wallclock Time: {}".format(t,
                                                                                                          current_time))
        ...
        if (print_volume_check == 1):
            ...
            if (detailed_check == 1):
                ...
                A_y_bottom = (0.0, y_c - A_param)  # bottom pole of cavity
